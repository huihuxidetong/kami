...
<body class="no-skin">
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'>${var.contacts_mobile}</td>
											<td class='center'>${var.data.uv}</td>
											<td class='center'>${var.data.auv}</td>
											<td class='center'>${var.data.bauv}</td>
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													<c:if test="${QX.cha == 1 }">
														<a class="btn btn-xs btn-success" onclick="detail('${var.id}');">详情</a>
													</c:if>
													<c:if test="${QX.edit == 1 }">
														<a class="btn btn-xs btn-success" onclick="edit('${var.id}');">编辑</a>
													</c:if>
													<c:if test="${QX.del == 1 }">
														<a class="btn btn-xs btn-danger" onclick="del('${var.id}');">删除</a>
													</c:if>
												</div>
	...
	<script type="text/javascript">
        ...
        //新增
        function add(){
            window.location.href="<%=basePath%>channel/goAdd.do";
        }
        //详情
        function detail(id) {
            window.location.href="<%=basePath%>channel/goDetail.do?id="+id;
        }
        //编辑
        function edit(id){
            window.location.href="<%=basePath%>channel/goEdit.do?id="+id;
        }
		...
		function del(id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>channel/delete.do?id="+id+"&tm="+new Date().getTime();
